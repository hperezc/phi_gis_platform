...
def register_callbacks(app):
    ...
    def load_filter_options(n_clicks):
        ...
        return [
            [{'label': str(x), 'value': x} for x in options.get('anos', pd.DataFrame({'ano': []})).ano],
    ...
    def update_all_components(start_date, end_date, ano, mes, zona, depto, municipio, 
        ...
        try:
            ...
            has_filters = any([ano, mes, zona, depto, municipio, categoria, grupo, grupo_intervencion, contrato, start_date, end_date])
            
            if not has_filters:
                # Sin filtros: crear gráficos vacíos con mensaje
                animated_fig = go.Figure()
                animated_fig.update_layout(title="Selecciona filtros para ver el análisis animado")
                bar_race_fig = go.Figure()
                bar_race_fig.update_layout(title="Aplica filtros para ver evolución temporal")
            else:
                # Con filtros: crear gráficos normales
