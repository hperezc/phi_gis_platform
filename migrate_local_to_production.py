#!/usr/bin/env python3
"""
Script de migración de base de datos local a producción
...
def copy_table_data(source_conn, dest_conn, table_name):
    ...
    try:
        ...
        if not rows:
            logger.info(f"Tabla {table_name} está vacía, saltando...")
            source_cur.close()
            return True
        ...
        dest_cur.close()
        logger.info(f"✅ Migración de tabla {table_name} completada: {total_rows} registros")
        return True
...
def create_table_if_not_exists(source_conn, dest_conn, table_name):
    ...
    try:
        ...
        logger.info(f"✅ Tabla {table_name} creada/verificada en destino")
        return True
...
def main():
    ...
    logger.info("🚀 Iniciando migración de base de datos local a producción")
    ...
    if not PRODUCTION_DB_URL:
        logger.error("❌ No se encontró DATABASE_URL en .env.production")
        sys.exit(1)
    
    logger.info(f"🔗 Conectando a base de datos local...")
    local_conn = get_db_connection(LOCAL_DB_URL)
    if not local_conn:
        logger.error("❌ No se pudo conectar a la base de datos local")
        sys.exit(1)
    
    logger.info(f"🔗 Conectando a base de datos de producción...")
    prod_conn = get_db_connection(PRODUCTION_DB_URL)
    if not prod_conn:
        logger.error("❌ No se pudo conectar a la base de datos de producción")
        local_conn.close()
        sys.exit(1)
    ...
    try:
        ...
        if not tables:
            logger.error("❌ No se encontraron tablas en la base de datos local")
            return
        ...
        logger.info("=" * 60)
        logger.info(f"✅ Tablas migradas exitosamente: {successful_migrations}")
        logger.info(f"❌ Tablas con errores: {len(failed_migrations)}")
        ...
        if successful_migrations > 0:
            ...
            logger.error("❌ No se pudo migrar ninguna tabla")
